...
- **Shell out to a local MTA (sendmail/postfix)** for delivery. A legitimate, common pattern (this is literally what PHP's `mail()` does) that offloads the hardest 20% (MX resolution, retry queues, greylisting, bounce generation, IP warm-up) to purpose-built software already on the host. Real cost: an operational dependency on a configured local MTA, and `Process`-based invocation needs its own async-safe wrapper (Foundation's `Process` is not concurrency-clean and is not `Sendable`). Kept as a **later, optional, explicitly lower-priority** transport (Phase 5) — not a substitute for the direct-MX transport, since once handed to `sendmail` this library's own TLS-policy/retry/DKIM guarantees no longer apply.
...
- **Hand-roll the SMTP wire protocol on `swift-nio` + `swift-nio-ssl`.** Matches Perfect-NIO's own established pattern in this exact ecosystem. Full control over the STARTTLS security invariant, connection pooling, pipelining, and DKIM signing pipeline ordering. **Chosen approach**, behind a pluggable `Transport` abstraction (§5.2) so the hardest piece (direct-MX delivery) ships as its own later phase rather than blocking an initial usable release.

## 4. Target architecture

### 4.1 Package / module layout

Three targets, three library products, `swift-tools-version: 6.2`, `platforms: [.macOS(.v26)]`, `.swiftLanguageMode(.v6)` on every target (matching Perfect-NIO's current baseline, not Perfect-Notifications' older `6.0`/`.v13` one):
...
PerfectSMTPLasso  // + PerfectSMTP. The Lasso dash-param compatibility overlay only.
                  // Perfect-Lasso takes a local path dependency on this target.
```
...
Three conforming strategies, all `Sendable`, shipping in different phases:

- **`RelayTransport`** (Phase 1) — owns a `SMTPConnectionPool` keyed to one configured smart host (SendGrid/Postmark/SES/corporate relay). First usable release.
- **`DirectMXTransport`** (Phase 3) — MX resolution, per-destination delivery, retry queue, greylisting, circuit-breaking, multi-host pool. The "primary point of delivery" story.
- **`LocalMTATransport`** (Phase 5) — async-safe wrapper around `sendmail`/`postfix -t`. A pragmatic, explicitly lower-priority, lower-guarantee alternative — see §3.
...
**Pipeline (plaintext / pre-upgrade):**

```
[socket] → SMTPResponseDecoder (ByteToMessageDecoder → SMTPReply)
         → SMTPClientHandler   (codec + promise-correlator ONLY — see revision below)
         ← SMTPRequestEncoder  (MessageToByteHandler)
```

For implicit TLS (port 465, RFC 8314's recommendation over STARTTLS/587 for submission), `NIOSSLClientHandler` is inserted at `.first` before the decoder at bootstrap — no in-band upgrade, no injection window at all.

**Reply decoding:** `SMTPResponseDecoder` accumulates lines until a terminal (non-`-`-continued) line, emits one `SMTPReply { code, enhancedStatus?, lines }`, and exposes whether residual bytes remain in its buffer after emitting a reply.
...
**PIPELINING:** when advertised, batch `MAIL FROM` + all `RCPT TO` + `DATA` in one flush, consume correlated replies in order; degrade to lock-step when absent. **Corrected semantics (from the protocol review):** if a `RCPT TO` in the middle of a pipelined batch returns a rejection (e.g. `550`), the remaining pipelined `RCPT` replies are still consumed in order; `DATA` proceeds only if at least one `RCPT` was accepted (otherwise the pipelined `DATA` itself draws `503`/`554` and the message body must never be sent); per-recipient `DeliveryResult`s reflect the actual mixed outcome. This must be an explicit, tested behavior (§8), not an incidental consequence of the handler's plumbing.

**Command/reply correlation — corrected design (concurrency review finding, critical).** The original architecture sketch described an "actor-owned" promise queue "fulfilled on the channel's event loop" — this is a genuine data race (two different execution contexts mutating the same collection with nothing synchronizing them), plus a separate write/enqueue ordering race if the promise is enqueued via a hop distinct from the write. **Corrected:** the correlation queue is **loop-confined, owned by the handler, never touched by the actor directly.** Each `SMTPCommand` value carries its own `EventLoopPromise<SMTPReply>` (created via `channel.eventLoop.makePromise`, which is `Sendable`); the handler's `write` appends the promise to its own loop-local FIFO *and* forwards the encoded bytes in the same call, so "enqueue" and "send" are atomic with respect to the event loop (the same atomicity PIPELINING's FIFO ordering already requires). The actor only ever does `try await promise.futureResult.get()`. **Mandatory, previously unspecified:** on `channelInactive`/`errorCaught`, the handler must fail every outstanding promise in its queue — otherwise a mid-conversation disconnect leaves the actor suspended forever. This is treated as the single most likely production hang and is a required automated test (§8).

**Decision boundary — corrected (concurrency review finding, high).** The original design was internally inconsistent about where protocol decisions happen (§3 implied the handler drives the state machine; §5's AUTH design said "the connection actor," not the handler). **Corrected, single model:** the handler is a dumb codec + promise correlator with **no protocol decisions in it at all**. All decision logic — EHLO capability interpretation, STARTTLS sequencing, SASL stepping including the `async` `XOAuth2.tokenProvider()` refresh callback, MAIL/RCPT/DATA ordering, the `535`/re-challenge retry — lives in the connection actor, which drives the conversation by sending commands and awaiting the correlated reply futures. This makes the XOAuth2 token-refresh callback safe: `await tokenProvider()` suspends the actor's task, never the event loop, and never happens while a reply promise is outstanding (the actor only sends the next AUTH line after the callback returns). The one pipeline mutation the actor needs (STARTTLS handler swap) is dispatched onto the loop via `channel.eventLoop.submit { }`, since `NIOChannelHandler.syncOperations` traps if called off-loop.

**STARTTLS upgrade — the security invariant, corrected and made testable.** Stated precisely: *no byte read from the socket before the TLS handshake may be processed as post-TLS input.* This defends against a real, current CVE class: CVE-2026-41319 in MailKit (April 2026 — pre-TLS buffered bytes survived into the post-handshake pipeline and forced a SASL mechanism downgrade), same lineage as Postfix CVE-2011-0411, Thunderbird CVE-2021-23993, Dovecot CVE-2021-33515.

Corrected sequence (two gaps closed from the original 6-step sketch, per the protocol review):
...
7. On handshake-complete: reset `capabilities` to empty, re-issue EHLO inside the encrypted channel, use only the post-TLS capability list for AUTH mechanism selection.

Automated regression test (§8) must feed the injected bytes in a **second, separate** inbound read/buffer (not concatenated into the same buffer as the `220`), since a single-buffer test only exercises the simpler case.

**`ChannelDuplexHandler` vs `NIOAsyncChannel` — rationale corrected.** The original justification ("`NIOAsyncChannel` hides byte-precise buffer control") isn't quite right — the residual-byte guarantee lives in the custom decoder regardless of what sits above it. The real, valid reason to use an explicit handler for the setup/EHLO/STARTTLS phase is that **mid-stream pipeline reconfiguration** (stop consuming inbound, remove the plaintext decoder, insert TLS, add a fresh decoder, resume) is more ergonomic with an explicit handler than with `NIOAsyncChannel`'s inbound `AsyncSequence`, which doesn't want its pipeline mutated mid-iteration. Worth evaluating during implementation: transitioning to `NIOAsyncChannel<SMTPReply, SMTPCommand>` **after** the TLS upgrade completes, for the bulk MAIL/RCPT/DATA/AUTH conversation where the hand-rolled promise queue carries the most incidental complexity and the least security justification.
...
**Two-phase (the Lasso `email_compose` analog):**
...
**Async job/track (the Lasso `email_result`/`email_status` analog):**
...
**Lasso-compat overlay** (`PerfectSMTPLasso`), one entry point mirroring Lasso 9's `email_send` signature:

```swift
public func emailSend(_ p: LassoEmailParams) async throws -> JobID
```

Mapping highlights (full parameter-by-parameter mapping in §6):
- `-to`/`-cc`/`-bcc` → each is a **single comma-delimited string** per both Lasso versions' documented behavior ("multiple `-To`, `-CC`, or `-BCC` parameters are not allowed"). **Corrected splitting logic (protocol review finding, required):** this must NOT be a naive `.split(separator: ",")` — RFC 5322 address lists routinely contain commas inside a quoted display-name phrase (`"Smith, John" <john@x.com>, "Doe, Jane" <jane@y.com>`), and a byte-level split on every comma produces broken, misdelivering address tokens. The overlay must use a proper RFC 5322 address-list tokenizer that treats comma as a separator only *outside* quoted-strings, comments, and angle-bracket address specs. Required test: a comma-in-display-name address list splits correctly. `-bcc` specifically routes to `SMTPEnvelope.recipients`, never to `EmailMessage`.
- `-htmlImages`/`-attachments` — these DO accept arrays (file paths, or `name=data` pairs), unlike `-to`/`-cc`/`-bcc`.
- `-tokens`/`-merge` → mail-merge templating: `-tokens` is a default value map applied to every message, `-merge` a per-recipient-address override map; `#name#` and `[Email_Token:'name']` placeholders are resolved **per recipient**, so each recipient becomes a distinct rendered `EmailMessage` and a distinct send.
- `-ssl` → maps to implicit TLS by default (closest match to Lasso 9's undifferentiated "SSL is used when connecting" documentation); the modern API still exposes the full `.plaintext`/`.startTLS`/`.implicit` enum for callers who want explicit control.
- `-clientIp` → the EHLO/HELO client identity (see §7's EHLO-hostname gap).
- `-immediate` → bypass the async job queue.
- `-ContentDisposition` (8.5-only) → `defaultDisposition`.
- The Lasso-documented 8MB size ceiling is modeled as a **configurable soft cap** on the overlay, not a hardcoded SMTP-layer rule — the real enforcement is the SIZE-capability fail-fast (§7).
...
- **Bug regressions as first-class, named tests:** (a) a Bcc address supplied via the overlay ends up in `SMTPEnvelope.recipients` and never in any serialized header, under `extraHeaders` too; (b) a non-ASCII subject is genuinely Q/B-encoded and round-trips correctly (the old `=?utf-8?Q?<raw>?=` output must fail this test).
...
- The Lasso address-list tokenizer against a comma-in-display-name fixture.
...
- **The STARTTLS buffer-discipline invariant as an automated regression** — the CVE-2026-41319-class test: feed `220 Ready\r\n` and a subsequent, separate injected read (`EHLO evil.example\r\n` as a **second** buffer, not concatenated) and assert the handler throws `.starttlsInjection`, closes the channel, and never processes the injected command; a companion test confirms the clean single-`220`-then-silence path upgrades correctly and re-issues EHLO with capabilities reset.
- Promise-queue failure-on-disconnect: a mid-conversation `channelInactive` must fail every outstanding correlated promise (§4.3) — an actor awaiting a reply must never hang forever.
...
## 6. Lasso 8.5 / Lasso 9 compatibility surface
...
- **Phase 5 — Lasso overlay & deliverability hygiene (low-medium risk).** `PerfectSMTPLasso` with the corrected quote-aware address-list splitting (§4.9), `-tokens`/`-merge` templating, the two-phase compose API, the async job/status API, List-Unsubscribe/-Post + Precedence/Auto-Submitted headers, and `LocalMTATransport`'s async-safe `Process` wrapper (which needs its own careful design: a single-resume guard bridging `terminationHandler` to a continuation, concurrent stdout/stderr draining to avoid a pipe-buffer deadlock, and confinement behind an actor since `Foundation.Process` is not `Sendable`).
...
- Perfect-Lasso's own native-function bindings for email sending should take a local path dependency on `PerfectSMTPLasso` once Phase 5 ships; that wiring is out of scope for this plan and belongs to a future Perfect-Lasso-side task.
